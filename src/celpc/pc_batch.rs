...
impl<'a> PolynomialProver<'a> {
    ...
    pub fn evaluate_batch(
        ...
        return BatchEvaluationProof {
            eval_pf: EvaluationProof { e: e, eps: eps },
...
impl<'a> PolynomialVerifier<'a> {
    pub fn verify_evaluation_batch(
        ...
        x1_big: &Integer,
        pc_batch: &BatchPolynomialCommitment,
        ep_batch: &BatchEvaluationProof,
    ) -> bool {
        let mut e_intt = ep_batch.eval_pf.e.clone();
        for p in e_intt.iter_mut() {
            self.params.ringq.intt(p);
        }
        let mut eps_intt = ep_batch.eval_pf.eps.clone();
        ...
        for i in (0..ep_batch.y_batch.len()).rev() {
            y_check *= x1_big;
            y_check += &ep_batch.y_batch[i];
            y_check.modulo_mut(&self.params.p);
        }
        if y_check != ep_batch.y_big {
            return false;
        }
        ...
        let mut commit_check = self
            .committer
            .commit(&ep_batch.eval_pf.e, &ep_batch.eval_pf.eps);
